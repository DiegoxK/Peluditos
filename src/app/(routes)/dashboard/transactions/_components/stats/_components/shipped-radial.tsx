...
export function ShippedRadial({ total, delivered }: ShippedRadialProps) {
  const chartData = [
    {
      delivered,
      total,
  ...
  return (
    <Card className="relative">
      <CardTitle className="text-primary flex items-center gap-2">
        <CheckCheck strokeWidth={2.2} />
        <span className="font-semibold">Ordenes entregadas</span>
      </CardTitle>
        ...
        >
          ...
          >
            <PolarGrid
              ...
              polarRadius={[86, 74]}
            />
            <RadialBar dataKey="delivered" background cornerRadius={10} />
                  ...
                  if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                    return (
                      ...
                      >
                        ...
                        >
                          {chartData[0]?.delivered?.toLocaleString()}
                        </tspan>
                        <tspan
                          x={viewBox.cx}
                          y={(viewBox.cy ?? 0) + 15}
      ...
      <div className="absolute bottom-21 left-1/2 ml-2 flex -translate-x-1/2 gap-6 text-sm">
        ...
        <span className="before:bg-chart-2 whitespace-nowrap before:mr-1 before:mb-[1px] before:inline-block before:h-2 before:w-2 before:rounded-md before:content-['']">
          {chartData[0]?.delivered} Entregadas
        </span>
      </div>
      <div className="mx-4">
        <Separator className="bg-border" />
